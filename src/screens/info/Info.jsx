...
export function Info() {
  const { t } = useT();
  const { property: p, content, publicMode } = useApp();
  const tip = content.arrivalTips[p.arrivalTipKey];
  const hi = content.houseInfo;
  const extra = {
    quiet: p.quietHours, cleaning: t('info.workdays'),
    parking: p.parkingSpots ? t('info.parkingSpots', { n: p.parkingSpots }) : null,
  };
  return (
    <>
      ...
      <Card style={{ marginBottom: 10 }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            {p.wifi.ssid ? (
              <div style={{ fontSize: 14, marginTop: 2 }}>{t('info.wifiName')}: <b style={{ fontFamily: monoFamily }}>{p.wifi.ssid}</b> · {t('info.wifiPass')}: <b style={{ fontFamily: monoFamily }}>{p.wifi.password}</b></div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {['kitchen', 'laundryRoom', 'quiet', 'cleaning', 'waste', 'smoking', 'visitors', 'parking', 'card'].map(k => (
          <InfoBlock key={k} icon={HOUSE_INFO_ICONS[k]} title={t('info.' + k) + (extra[k] ? ' · ' + extra[k] : '')}>{hi[k]}</InfoBlock>
        ))}
